  ...
  for(const m of messages){
   try{
    ...
    if(selectedButton){
      ...
      if(selectedButton==="menu_stats"){if(!isOwner(sender)){await send(sock,jid,"⛔ Owner only.",{category:"admin"});continue;}const st=await db.stats();await send(sock,jid,"📊 *TOHID-AGENT V4 STATS*\\nUsers: "+(st.users??"N/A")+"\\nBlocked: "+(st.blocked??0)+"\\nChat: "+(st.usage?.chat??0)+"\\nVoice: "+(st.usage?.voice??0)+"\\nImages: "+(st.usage?.image??0)+"\\nVideos: "+(st.usage?.video??0),{category:"stats"});continue;}
      if(selectedButton==="menu_settings"){await menu.sendMenu(sock,jid,"settings",{text:"⚙️ *TOHID-AGENT V4 SETTINGS*\\nUI: "+(useButtons?"BUTTONS":"TEXT")+"\\nVoice: "+(s.voice===true?"ON":s.voice===false?"OFF":cfg.voiceReply?"ON (global)":"OFF")+"\\nMemory: "+(s.memory===false?"OFF":"ON")});continue;}
      ...
      if(selectedButton==="settings_buttons"||selectedButton==="settings_text"){const buttons=selectedButton.endsWith("_buttons");await db.setSettings(sender,{uiMode:buttons?"buttons":"text"});if(buttons)await menu.sendMenu(sock,jid,"main",{text:"🔘 Button mode enabled. All menus will use buttons when supported."});else await send(sock,jid,"📝 Text mode enabled. Use .menu and text commands instead of buttons.");continue;}
