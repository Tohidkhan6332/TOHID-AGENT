...
async function sendMain(sock,jid){
  ...
  if(cfg.channelLink){
    buttons.unshift(makeUrlButton("↗ 1st-Channel",cfg.channelLink));
  }
  if(cfg.channelLink2){
    buttons.splice(Math.min(1,buttons.length),0,makeUrlButton("↗ 2nd-Channel",cfg.channelLink2));
  }
  if(cfg.groupLink){
    const openIndex=buttons.findIndex(x=>x?.buttonParamsJson&&x.buttonParamsJson.includes("tohid_menu"));
    buttons.splice(openIndex<0?buttons.length:openIndex,0,makeUrlButton("👥 WhatsApp Group",cfg.groupLink));
  }
  return relayInteractive(sock,jid,{
    header:"🤖 TOHID-AGENT",
...
module.exports={relayInteractive,sendLanguageMenu,sendHelp,sendHelpByMode,sendLanguageMenuByMode,getInteractiveId,actionToText,sendActionConfirmation,sendCommandMenu,sendMain,sendList,sendDev,sendSettings,sendTextMenu,sendMenuByMode};
