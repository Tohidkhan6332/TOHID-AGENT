...
async function sendHelp(sock,jid,language="en"){
  const english =
    "🤖 *TOHID-AGENT V9.0 — COMPLETE HELP*\\n\\n"+
    "👨‍💻 Developer: Tohid\\n"+
...
async function sendMain(sock,jid){
  return localizedMenu(sock,jid,"🤖 TOHID-AGENT","Choose what you want TOHID-AGENT to do.",[
    {text:"🤖 AI Chat",id:"tohid_ai"},{text:"🐙 GitHub",id:"tohid_github"},{text:"🚀 Heroku",id:"tohid_heroku"}
  ]);
}
async function sendList(sock,jid){
  return localizedMenu(sock,jid,"🧰 TOHID-AGENT TOOLS","Choose a tool or feature.",[
    {text:"🧭 Planner",id:"tohid_plan"},{text:"📖 Help",id:"tohid_help_languages"},{text:"⚙️ Settings",id:"tohid_settings"}
  ]);
}
async function sendDev(sock,jid){
  return localizedMenu(sock,jid,"👨‍💻 Developer Tools","Use natural language to manage GitHub and supported hosting platforms.",[
    {text:"🐙 GitHub",id:"tohid_github"},{text:"🚀 Heroku",id:"tohid_heroku"},{text:"📢 Channel",id:"tohid_channel"}
  ]);
}
async function sendSettings(sock,jid){
  return localizedMenu(sock,jid,"⚙️ Settings","Change voice, memory or bot language.",[
    {text:"🎙️ Voice ON",id:"tohid_voice_on"},{text:"🧠 Memory ON",id:"tohid_memory_on"},{text:"🌐 Language",id:"tohid_help_languages"}
  ]);
}
...
async function sendTextMenu(sock,jid,kind="main"){
  const language=await i18n.getLanguage(jid);
  const menus={
    main:["🤖 TOHID-AGENT","1. AI Chat — send any message","2. GitHub — manage repositories naturally","3. Heroku — manage configured apps","4. Tools — planner/help/settings","5. Language — change bot language"],
    list:["🧰 TOHID-AGENT TOOLS","1. Planner — .plan <task>","2. Help — .help","3. Settings — .settings","4. Language — .language <language>"],
    dev:["👨‍💻 Developer Tools","1. GitHub — ask naturally","2. Heroku — ask naturally","3. Channel — use .channel"],
    settings:["⚙️ Settings","1. Voice — .voice on/off","2. Memory — .memory on/off","3. Language — .language <language>","4. UI Mode — .mode buttons/text/both"]
  };
  const lines=menus[kind]||menus.main;
  const text=await i18n.translate(lines.join("\n"),language);
  return sock.sendMessage(jid,{text});
}

async function sendMenuByMode(sock,jid,kind="main",mode="both"){
  const m=["buttons","text","both"].includes(String(mode).toLowerCase())?String(mode).toLowerCase():"both";
  if(m==="text")return sendTextMenu(sock,jid,kind);
  if(m==="buttons")return kind==="main"?sendMain(sock,jid):kind==="list"?sendList(sock,jid):kind==="dev"?sendDev(sock,jid):sendSettings(sock,jid);
  await sendTextMenu(sock,jid,kind);
  return kind==="main"?sendMain(sock,jid):kind==="list"?sendList(sock,jid):kind==="dev"?sendDev(sock,jid):sendSettings(sock,jid);
}


async function sendHelpByMode(sock,jid,language,mode="both"){
  const m=["buttons","text","both"].includes(String(mode).toLowerCase())?String(mode).toLowerCase():"both";
  if(m==="text"){
    const lang=language||await i18n.getLanguage(jid);
    const english="🤖 TOHID-AGENT V9.0 — COMPLETE HELP\n\nUse .menu, .mode buttons, .mode text or .mode both. Use .language <language> to change the bot language.";
    return sock.sendMessage(jid,{text:await i18n.translate(english,lang)});
  }
  if(m==="buttons")return sendHelp(sock,jid,language);
  const lang=language||await i18n.getLanguage(jid);
  const translated=await i18n.translate("🤖 TOHID-AGENT V9.0 — COMPLETE HELP\n\nUse the buttons below or type commands/text directly. Use .mode buttons, .mode text or .mode both to choose the interface.",lang);
  await sock.sendMessage(jid,{text:translated});
  return sendHelp(sock,jid,language);
}

module.exports={relayInteractive,sendLanguageMenu,sendHelp,sendHelpByMode,getInteractiveId,actionToText,sendMain,sendList,sendDev,sendSettings,sendTextMenu,sendMenuByMode};
