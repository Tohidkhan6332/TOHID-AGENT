...
async function sendMenuByMode(sock,jid,kind="main",mode="hybrid"){
  ...
  if(m==="text"||m==="minimal"){
    if(m==="minimal"){
      const language=await i18n.getLanguage(jid);
      const minimal=await i18n.translate(kind==="main"?"🤖 TOHID-AGENT\\nSend a message or use .help":kind==="settings"?"⚙️ Settings\\n.voice on/off • .memory on/off • .mode <mode>":"🧰 TOHID-AGENT\\nUse .help for commands.",language);
      return sock.sendMessage(jid,{text:minimal});
    }
    return sendTextMenu(sock,jid,kind);
  }
...
async function sendHelpByMode(sock,jid,language,mode="hybrid"){
  const m=String(mode||"hybrid").toLowerCase();
  if(m==="text"||m==="minimal"){
    const lang=language||await i18n.getLanguage(jid);
    const english=m==="minimal"?"🤖 TOHID-AGENT V10\\nUse .help, .menu, .mode <mode> or send a message.":"🤖 TOHID-AGENT V10 — COMPLETE HELP\\n\\nUse .menu, .mode auto, .mode buttons, .mode text, .mode hybrid or .mode minimal. Use .language <language> to change the bot language.";
    return sock.sendMessage(jid,{text:await i18n.translate(english,lang)});
  }
  if(m==="buttons")return sendHelp(sock,jid,language);
  const lang=language||await i18n.getLanguage(jid);
  const translated=await i18n.translate("🤖 TOHID-AGENT V10 — COMPLETE HELP\n\nUse the buttons below or type commands/text directly. Auto mode chooses the least noisy interface for each situation.",lang);
