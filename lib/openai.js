...
if(writes.has(name)){
 ...
 pendingWrites.set(userId,{name,args});await db.savePending(userId,{name,args});
...
const p=pendingWrites.get(userId)||await db.getPending(userId);if(!p)return null;
...
if(isCancellation(text)&&(pendingWrites.has(userId)||await db.getPending(userId))){pendingWrites.delete(userId);await db.savePending(userId,null);return"🛑 GitHub action cancelled.";}
