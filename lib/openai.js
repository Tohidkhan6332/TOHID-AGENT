...
if(writes.has(name)){
 ...
 pendingWrites.set(userId,{name,args});
...
const p=pendingWrites.get(userId);if(!p)return null;
pendingWrites.delete(userId);
return runTool(p.name,p.args);
}
...
if(isConfirmation(text)&&pendingWrites.has(userId)){const result=await executePending(userId);await db.track(userId,"github_write");return"✅ GitHub action completed by TOHID-AGENT.\n\n"+JSON.stringify(result,null,2);}
...
if(!calls.length){const answer=(r.data.output_text||"").trim();await remember(userId,"user",text);await remember(userId,"assistant",answer);await db.track(userId,"chat");return answer||"I couldn't generate a response.";}
