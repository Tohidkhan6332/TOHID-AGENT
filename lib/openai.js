...
{type:"function",name:"whatsapp_channel",description:"Inspect or manage a WhatsApp Channel/newsletter. Read actions: info, subscribers. Mutating actions: follow, unfollow, mute, unmute. Protected owner action requiring explicit CONFIRM.",parameters:{type:"object",properties:{action:{type:"string",enum:["info","follow","unfollow","mute","unmute","subscribers"]},channelJid:{type:"string"}},required:["action","channelJid"],additionalProperties:false}},
{type:"function",name:"whatsapp_group_status",description:"Send a WhatsApp group status message to the current group. Protected owner action requiring explicit CONFIRM.",parameters:{type:"object",properties:{text:{type:"string"}},required:["text"],additionalProperties:false}},
...
case "whatsapp_channel":{
  const mutating=["follow","unfollow","mute","unmute"].includes(args.action);
  ...
  const map={info:"newsletterMetadata",follow:"newsletterFollow",unfollow:"newsletterUnfollow",mute:"newsletterMute",unmute:"newsletterUnmute",subscribers:"newsletterSubscribers"};
  return runtime.baileysExtras.newsletter(runtime.sock,map[args.action],...(args.action==="info"?["jid",args.channelJid]:[args.channelJid]));
}
...
async function askGemini(userId,text,{isOwner=false,imageData=null,baileysExtras=null,sock=null,jid=null}={}) {
