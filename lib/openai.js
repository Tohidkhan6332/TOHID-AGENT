...
async function runTool(name,args){switch(name){
...
for(let round=0;round<8;round++){const r=await axios.post("https://api.openai.com/v1/responses",body,{headers:jsonHeaders(),timeout:120000});const out=r.data.output||[],calls=out.filter(x=>x.type==="function_call");if(!calls.length){const answer=(r.data.output_text||"").trim();if(useMemory){await remember(userId,"user",text);await remember(userId,"assistant",answer);}await db.track(userId,"chat");return answer||"I couldn't generate a response.";}for(const call of calls){let result;try{result=await githubTool(call.name,JSON.parse(call.arguments||"{}"),isOwner,userId);}catch(e){result={error:e.message};}body.input.push({type:"function_call_output",call_id:call.call_id,output:JSON.stringify(result)});}}
...
async function askGemini(userId,text,{isOwner=false,imageData=null}={}) {
  ...
  for(let round=0;round<8;round++){
    ...
    const r=await axios.post("https://generativelanguage.googleapis.com/v1beta/openai/chat/completions",body,{headers:geminiHeaders(),timeout:120000});
...
async function ttsGemini(text,out){
  ...
  const r=await axios.post("https://generativelanguage.googleapis.com/v1beta/interactions",
    ...
    {headers:{"x-goog-api-key":cfg.geminiKey,"Content-Type":"application/json","Api-Revision":"2026-05-20"},timeout:120000});
...
async function videoGemini(prompt){
  ...
  const created=await axios.post("https://generativelanguage.googleapis.com/v1beta/openai/videos",form,{headers:{Authorization:"Bearer "+cfg.geminiKey,...form.getHeaders()},timeout:120000});
