...
function remember(id,role,content){const h=history(id);h.push({role,content});memory.set(id,h.slice(-20));}
...
async function ask(userId,text,{isOwner=false}={}){
 ...
 for(let round=0;round<8;round++){
  ...
  if(!calls.length){const answer=(r.data.output_text||"").trim();remember(userId,"assistant",answer);return answer||"I couldn't generate a response.";}
...
async function transcribe(file){
 ...
 const r=await axios.post("https://api.openai.com/v1/audio/transcriptions",form,{headers:{Authorization:"Bearer "+cfg.openaiKey,...form.getHeaders()},timeout:120000});return r.data.text;
}
async function tts(text,out){
 const r=await axios.post("https://api.openai.com/v1/audio/speech",{model:cfg.ttsModel,voice:cfg.ttsVoice,input:text,format:"mp3"},{headers:jsonHeaders(),responseType:"arraybuffer",timeout:120000});fs.writeFileSync(out,r.data);return out;
}
async function image(prompt){
 ...
 const b64=r.data.data?.[0]?.b64_json;if(!b64)throw new Error("Image API did not return image bytes.");
...
async function video(prompt){
 if(!cfg.videoEnabled)throw new Error("Video generation is disabled. Set AI_VIDEO_ENABLED=true.");
 const form=new FormData();
 form.append("model",cfg.videoModel);form.append("prompt",prompt);form.append("seconds",String(cfg.videoSeconds));form.append("size",cfg.videoSize);
 const created=await axios.post("https://api.openai.com/v1/videos",form,{headers:{Authorization:"Bearer "+cfg.openaiKey,...form.getHeaders()},timeout:120000});
 const id=created.data.id;if(!id)throw new Error("Video API did not return a job ID.");
 let job=created.data;
 for(let i=0;i<90 && ["queued","in_progress"].includes(job.status);i++){
  await new Promise(r=>setTimeout(r,10000));
  const status=await axios.get("https://api.openai.com/v1/videos/"+id,{headers:{Authorization:"Bearer "+cfg.openaiKey},timeout:60000});
  job=status.data;
  if(job.status==="failed")throw new Error(job.error?.message||"Video generation failed.");
 }
 if(job.status!=="completed")throw new Error("Video generation timed out. Job: "+id);
 const out=path.join(process.cwd(),"tmp-video-"+Date.now()+".mp4");
 const media=await axios.get("https://api.openai.com/v1/videos/"+id+"/content",{headers:{Authorization:"Bearer "+cfg.openaiKey},responseType:"arraybuffer",timeout:180000});
 fs.writeFileSync(out,media.data);return out;
}
module.exports={ask,transcribe,tts,image,video};
