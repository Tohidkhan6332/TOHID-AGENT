...
if(writes.has(name)){
 ...
 pendingWrites.set(userId,{name,args,createdAt:Date.now()});await db.savePending(userId,{name,args,createdAt:Date.now()});await db.audit(userId,"github_pending",{name,args});
 return{confirmation_required:true,action:name,args,message:"Protected GitHub write prepared. Reply CONFIRM to execute this exact action, or CANCEL to stop."};
}
return runTool(name,args);
}
async function executePending(userId){
const p=pendingWrites.get(userId)||await db.getPending(userId);if(!p)return null;if(p.createdAt&&Date.now()-p.createdAt>cfg.pendingActionTtlMs){pendingWrites.delete(userId);await db.savePending(userId,null);await db.audit(userId,"github_pending_expired",{name:p.name});return {expired:true};}
pendingWrites.delete(userId);await db.savePending(userId,null);
const result=await runTool(p.name,p.args);await db.audit(userId,"github_write_executed",{name:p.name,args:p.args});return result;
}
...
if(!calls.length){const answer=(r.data.output_text||"").trim();if(useMemory){await remember(userId,"user",text);await remember(userId,"assistant",answer);}await db.track(userId,"chat");return answer||"I couldn't generate a response.";}
