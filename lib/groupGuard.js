...
async function handleCommand({sock,msg,jid,sender,text,db,send}){
  ...
  if((name==="welcome"||name==="goodbye")&&["add","remove","list","clear"].includes(sub)){
    ...
    if(sub==="remove"){
      const index=Number(String(parts[2]||""));
      if(!Number.isInteger(index)||index<1||index>rule.messages.length){await send(sock,jid,"❌ Invalid message number. Use ."+name+" list first.",{category:"utility"});return true;}
...
async function handleParticipantUpdate({sock,id,participants,action,db,send,author,authorPn}){
  ...
  if((action==="add"&&s.welcome.enabled)||(action==="remove"&&s.goodbye.enabled)){
    ...
    if(targets.length){
      ...
      try{
        if(selected?.image)await sock.sendMessage(id,{image:{url:selected.image},caption:selected.text||defaultBody,mentions});
        else await send(sock,id,selected?.text||defaultBody,{mentions,category:"utility"});
        handled=true;
