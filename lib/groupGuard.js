...
async function enforce({sock,msg,jid,sender,reason,action,meta,db,send}){
  if(isAdmin(meta,sender)||isAdmin(meta,botJid(sock))&&isAdmin(meta,sender))return false;
...
async function handleParticipantUpdate({sock,id,participants,action,db,send}){\n  if(!id?.endsWith("@g.us")||action!=="add")return false;\n  const s=await settings(db,id);\n  if(!s.antibot.enabled||(s.antibot.numbers||[]).length===0)return false;\n  let meta;try{meta=await sock.groupMetadata(id);}catch{return false;}\n  if(!isBotAdmin(meta,sock))return false;\n  for(const target of participants||[]){\n    const number=String(target).split("@")[0].replace(/\\D/g,"");\n    if(!(s.antibot.numbers||[]).includes(number))continue;\n    try{await sock.groupParticipantsUpdate(id,[target],"remove");await send(sock,id,"🚫 AntiBot: configured bot removed ("+number+").",{category:"security"});}catch(e){await send(sock,id,"❌ AntiBot could not remove "+number+": "+e.message,{category:"error"});}\n  }\n  return true;\n}\n\nasync function inspect({sock,msg,jid,sender,text,db,send}){
  if(!jid.endsWith("@g.us"))return false;
  const s=await settings(db,jid);
