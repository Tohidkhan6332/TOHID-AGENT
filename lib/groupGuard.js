...
const advanced=require("./groupAdvanced");
const COMMANDS=new Set(["antilink","antistatus","antigroupstatus","antibot","antitag","antitagall","antipdm","antibad","welcome","goodbye","security","warn","warnings","resetwarn","warnlimit","modlog","group","lock","unlock","groupstats","member","verify","antispam","mute","unmute","muted","raid","verification","schedule","activity","topchatters","modstats"]);
...
function defaults(){
  return {
    ...
    antigroupstatus:{enabled:false,action:"delete",allowed:[]},
...
async function settings(db,jid){
  ...
  return {...defaults(),...current,
    ...
    antigroupstatus:{...defaults().antigroupstatus,...(current.antigroupstatus||{}),allowed:Array.isArray(current.antigroupstatus?.allowed)?current.antigroupstatus.allowed:[]},
...
async function enforce({sock,msg,jid,sender,reason,action,meta,db,send}){
  ...
  const current=await settings(db,jid);
  const ruleKey=reason.startsWith("Lock:")?null:reason.startsWith("AntiLink")?"antilink":reason.startsWith("AntiStatus")?"antistatus":reason.startsWith("AntiTagAll")?"antitagall":reason.startsWith("AntiTag")?"antitag":reason.startsWith("AntiBot")?"antibot":"antibad";
  if(ruleKey&&isAllowed(meta,current[ruleKey],sender))return false;
  ...
  try{
    if(act==="delete")await sock.sendMessage(jid,{delete:msg.key});
      ...
      if(count>=(Number(current.security?.warningLimit)||3)){await sock.sendMessage(jid,{delete:msg.key});await sock.groupParticipantsUpdate(jid,[sender],"remove");await clearWarning(db,jid,sender);await send(sock,jid,"🚫 @"+String(sender).split("@")[0]+" removed after "+(Number(current.security?.warningLimit)||3)+" warnings ("+reason+").",{mentions:[sender],category:"security"});}
      else await send(sock,jid,"⚠️ @"+String(sender).split("@")[0]+" warning "+count+"/"+(Number(current.security?.warningLimit)||3)+" — "+reason,{mentions:[sender],category:"security"});
    }
...
async function handleCommand({sock,msg,jid,sender,text,db,send}){
  ...
  if(await advanced.handleCommand({sock,msg,jid,sender,text,db,send}))return true;
...
async function handleParticipantUpdate({sock,id,participants,action,db,send,author,authorPn}){
  ...
  await advanced.handleParticipantUpdate({sock,id,participants,action,db,send});
  ...
  if(action==="add"&&policy.isLocked(s,"newmembers")&&isBotAdmin(meta,sock)){
    try{await sock.groupParticipantsUpdate(id,(participants||[]).filter(Boolean),"remove");await send(sock,id,"🔒 New members are temporarily locked for this group.",{category:"security"});handled=true;}catch(e){await send(sock,id,"❌ New-member lock enforcement failed: "+e.message,{category:"error"});}
  }
...
async function inspect({sock,msg,jid,sender,text,db,send}){
  if(!jid.endsWith("@g.us"))return false;
  if(await advanced.inspect({sock,msg,jid,sender,text,db,send}))return true;
  const s=policy.cleanup(await settings(db,jid));
  const enabled=Object.values(s).some(v=>v&&typeof v==="object"&&v.enabled)||s.security.locks.length>0;
  if(!enabled)return false;
  let meta;try{meta=await sock.groupMetadata(jid);}catch{return false;}
  if(isAdmin(meta,sender))return false;
  if(policy.isLocked(s,"links")&&URL_RE.test(String(text||"")))return enforce({sock,msg,jid,sender,reason:"Lock: links are disabled",action:"delete",meta,db,send});
  if(policy.isLocked(s,"media")&&/imageMessage|videoMessage|documentMessage|audioMessage/i.test(JSON.stringify(unwrap(msg.message))))return enforce({sock,msg,jid,sender,reason:"Lock: media is disabled",action:"delete",meta,db,send});
  if(policy.isLocked(s,"images")&&/imageMessage/i.test(JSON.stringify(unwrap(msg.message))))return enforce({sock,msg,jid,sender,reason:"Lock: images are disabled",action:"delete",meta,db,send});
  if(policy.isLocked(s,"video")&&/videoMessage/i.test(JSON.stringify(unwrap(msg.message))))return enforce({sock,msg,jid,sender,reason:"Lock: video is disabled",action:"delete",meta,db,send});
  if(policy.isLocked(s,"audio")&&/audioMessage/i.test(JSON.stringify(unwrap(msg.message))))return enforce({sock,msg,jid,sender,reason:"Lock: audio is disabled",action:"delete",meta,db,send});
  if(policy.isLocked(s,"voice")&&/audioMessage[^]*"ptt":true/i.test(JSON.stringify(unwrap(msg.message))))return enforce({sock,msg,jid,sender,reason:"Lock: voice is disabled",action:"delete",meta,db,send});
  if(policy.isLocked(s,"documents")&&/documentMessage/i.test(JSON.stringify(unwrap(msg.message))))return enforce({sock,msg,jid,sender,reason:"Lock: documents are disabled",action:"delete",meta,db,send});
  if(policy.isLocked(s,"polls")&&/pollCreationMessage|pollUpdateMessage/i.test(JSON.stringify(unwrap(msg.message))))return enforce({sock,msg,jid,sender,reason:"Lock: polls are disabled",action:"delete",meta,db,send});
  if(policy.isLocked(s,"contacts")&&/contactMessage|contactsArrayMessage/i.test(JSON.stringify(unwrap(msg.message))))return enforce({sock,msg,jid,sender,reason:"Lock: contacts are disabled",action:"delete",meta,db,send});
  if(policy.isLocked(s,"locations")&&/locationMessage|liveLocationMessage/i.test(JSON.stringify(unwrap(msg.message))))return enforce({sock,msg,jid,sender,reason:"Lock: locations are disabled",action:"delete",meta,db,send});
  if(policy.isLocked(s,"mentions")&&mentions(msg.message).length)return enforce({sock,msg,jid,sender,reason:"Lock: mentions are disabled",action:"delete",meta,db,send});
  if(policy.isLocked(s,"stickers")&&/stickerMessage/i.test(JSON.stringify(unwrap(msg.message))))return enforce({sock,msg,jid,sender,reason:"Lock: stickers are disabled",action:"delete",meta,db,send});
  if(policy.isLocked(s,"forwards")&&contextInfo(msg.message).isForwarded)return enforce({sock,msg,jid,sender,reason:"Lock: forwards are disabled",action:"delete",meta,db,send});
  if(policy.isLocked(s,"all")&&/imageMessage|videoMessage|audioMessage|documentMessage|stickerMessage|pollCreationMessage|contactMessage|locationMessage/i.test(JSON.stringify(unwrap(msg.message))))return enforce({sock,msg,jid,sender,reason:"Lock: all content is disabled",action:"delete",meta,db,send});
  const badRule=policy.effectiveRule(s,"antibad");
  if(badRule.enabled&&!isAllowed(meta,s.antibad,sender)){const hit=badWordHit(text,s.antibad.words);if(hit)return enforce({sock,msg,jid,sender,reason:"AntiBad: inappropriate word detected",action:badRule.action,meta,db,send});}
  const linkRule=policy.effectiveRule(s,"antilink");
  if(linkRule.enabled&&!isAllowed(meta,s.antilink,sender)&&URL_RE.test(String(text||"")))return enforce({sock,msg,jid,sender,reason:"AntiLink: link detected",action:linkRule.action,meta,db,send});
  const statusRule=policy.effectiveRule(s,"antigroupstatus").enabled?policy.effectiveRule(s,"antigroupstatus"):policy.effectiveRule(s,"antistatus");
  if(statusRule.enabled&&!isAllowed(meta,statusRule,sender)&&hasStatusMention(msg.message))return enforce({sock,msg,jid,sender,reason:"AntiStatus: status mention detected",action:statusRule.action,meta,db,send});
  const allRule=policy.effectiveRule(s,"antitagall");
  if(allRule.enabled&&!isAllowed(meta,s.antitagall,sender)&&hasMentionAll(msg.message,text,meta))return enforce({sock,msg,jid,sender,reason:"AntiTagAll: mass mention detected",action:allRule.action,meta,db,send});
  const tagRule=policy.effectiveRule(s,"antitag");
  if(tagRule.enabled&&!isAllowed(meta,s.antitag,sender)&&mentions(msg.message).length)return enforce({sock,msg,jid,sender,reason:"AntiTag: mention detected",action:tagRule.action,meta,db,send});
  const botRule=policy.effectiveRule(s,"antibot");
  if(botRule.enabled&&!isAllowed(meta,s.antibot,sender)){
    const number=String(sender).split("@")[0].replace(/\D/g,"");
    if((s.antibot.numbers||[]).includes(number))return enforce({sock,msg,jid,sender,reason:"AntiBot: configured bot detected",action:botRule.action,meta,db,send});
  }
  return false;
}
