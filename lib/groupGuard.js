...
const COMMANDS=new Set(["antilink","antistatus","antibot","antitag","antitagall","antipdm","antibad"]);
...
function defaults(){
  return {
    ...
    antibad:{enabled:false,action:"delete",words:[]},
    warnings:{}
  };
}
async function settings(db,jid){
  ...
  return {...defaults(),...current,
    ...
    antibad:{...defaults().antibad,...(current.antibad||{})},
...
async function inspect({sock,msg,jid,sender,text,db,send}){
  ...
  if(isAdmin(meta,sender))return false;
  if(s.antibad.enabled){const hit=badWordHit(text,s.antibad.words);if(hit)return enforce({sock,msg,jid,sender,reason:"AntiBad: inappropriate word detected",action:s.antibad.action,meta,db,send});}
