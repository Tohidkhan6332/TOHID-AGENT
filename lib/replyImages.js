...
function getImage(category){if(!ENABLED)return null;const preferred=path.join(ROOT,FILES[category]||FILES.ai);if(fs.existsSync(preferred))return preferred;const fallback=Object.values(FILES).map(name=>path.join(ROOT,name)).find(file=>fs.existsSync(file));return fallback||null;}
