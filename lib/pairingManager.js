...
function create(options={}){
  ...
  if(!validPhone(phone))throw new Error("Phone number must contain country code and 10-15 digits.");
  const mode=String(options.mode||"pairing").toLowerCase()==="qr"?"qr":"pairing";
  if([...sessions.values()].some(s=>s.phone===phone&&["starting","pairing","connected"].includes(s.status)))throw new Error("A session for this number is already active.");
  ...
  const pairingCode=String(Math.floor(10000000+Math.random()*90000000));
  const childEnv={
    ...process.env,
    ...env,
    OWNER_NUMBER:phone,
