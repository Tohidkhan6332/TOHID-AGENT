...
const allowedEnv=new Set([
  ...
  "MONGO_URI","MONGO_DB","POSTGRES_URL","POSTGRES_SSL","POSTGRES_POOL_MAX","BOT_LANGUAGE","BOT_UI_MODE","PREFIX",
...
function snapshot(s){
  return {id:s.id,phone:s.phone,mode:s.mode,status:s.status,code:s.code||null,qr:s.qr||null,connected:!!s.connected,sessionId:s.sessionId||null,sessionError:s.sessionError||null,createdAt:s.createdAt,updatedAt:s.updatedAt};
}
function canAccess(s,ownerId){return !ownerId||!s.ownerId||String(s.ownerId)===String(ownerId);}
...
function create(options={}){
  ...
  delete childEnv.SESSION_ID;
  delete childEnv.PAIRING_WEB_TOKEN;
  delete childEnv.TELEGRAM_BOT_TOKEN;
  const child=spawn(process.execPath,[path.join(process.cwd(),"MrTohid.js")],{
    cwd:process.cwd(),env:childEnv,stdio:["ignore","pipe","pipe","ipc"]
  });
  const session={id,phone,mode,ownerId:options.ownerId||null,env,status:"starting",code:null,qr:null,connected:false,child,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};
  ...
  session.connected=true;session.status="connected";touch();
  sessionStore.createFromAuthDir(authDir,{phone:session.phone}).then(async id=>{
    session.sessionId=id;session.status="ready";session.sessionError=null;touch();
    try{child.kill("SIGTERM");}catch{}
    setTimeout(()=>{try{if(!child.killed)child.kill("SIGKILL");}catch{}},5000);
    try{fs.rmSync(sessionDir(session.id),{recursive:true,force:true});}catch{}
  }).catch(e=>{
    session.status="error";session.sessionError=e?.message||"Session storage failed";touch();
  });
}
...
async function stop(id,ownerId=null){
  const s=sessions.get(String(id));if(!s)return false;
  if(!canAccess(s,ownerId))return false;
...
function list(ownerId=null){return [...sessions.values()].filter(s=>canAccess(s,ownerId)).map(snapshot);}
...
async function exportSession(id,ownerId=null){
  const s=sessions.get(String(id));
  if(!s||!canAccess(s,ownerId))return null;
  if(!s.connected)throw new Error("WhatsApp session is not connected yet.");
  if(s.sessionId)return s.sessionId;
  s.sessionId=await sessionStore.createFromAuthDir(path.join(sessionDir(s.id),"auth"),{phone:s.phone});
  s.status="ready";s.updatedAt=new Date().toISOString();
  return s.sessionId;
}
module.exports={create,get,getSession,stop,list,stats,exportSession,cleanEnv,allowedEnv};
