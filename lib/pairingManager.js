...
function snapshot(s){
  return {id:s.id,phone:s.phone,mode:s.mode,status:s.status,code:s.code||null,qr:s.qr||null,connected:!!s.connected,sessionId:s.sessionId||null,createdAt:s.createdAt,updatedAt:s.updatedAt};
}
...
function create(options={}){
  ...
  try{session.sessionId=sessionManager.encodeAuthDir(authDir);touch();}
  catch(e){session.status="connected";session.sessionError=e?.message||"Session export failed";touch();}
}
...
function exportSession(id,ownerId=null){
  const s=sessions.get(String(id));
  if(!s||!canAccess(s,ownerId))return null;
  if(!s.connected)throw new Error("WhatsApp session is not connected yet.");
  if(!s.sessionId)s.sessionId=sessionManager.encodeAuthDir(path.join(sessionDir(s.id),"auth"));
  return s.sessionId;
}
