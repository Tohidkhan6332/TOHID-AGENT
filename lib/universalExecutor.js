...
const MEDIA_COMMANDS=new Set(["play","song","video","ytmp3","ytmp4","facebook","instagram","tiktok","twitter","pinterest","spotify","mediafire","apk"]);
...
async function execute({command,args=[],sock,jid,sender,isOwner,hasPermission}){
  ...
  if(MEDIA_COMMANDS.has(name)){
    ...
    if(!input)return {handled:true,text:"📥 *"+name+"* ready.\n\nUsage: ."+name+" <media URL>\n\nProvider: "+providers.MEDIA_ENDPOINT};
    try{
      const result=await providers.downloadMedia(input);
  ...
  if(["joke","quote","fact","truth","dare","8ball"].includes(name)){
    const answers={
      ...
      quote:"Build systems that are understandable before making them clever.",
      fact:"A WhatsApp group can have different moderation policy layers without needing a second socket.",
      truth:"Truth: What is one project you want to finish this week?",
      dare:"Dare: Refactor one messy function today.",
      "8ball":"🎱 Ask a yes/no question."
    };
    return {handled:true,text:answers[name]};
  }

  if(["quiz","trivia"].includes(name))
    return {handled:true,text:"🎮 Quiz command is registered. The existing TOHID quiz engine can be connected to this universal route."};
  ...
  if(["kick","add","promote","demote","tagall","hidetag","groupinfo","admins","link","revoke","setname","setdesc"].includes(name)){
    ...
    if(name==="groupinfo"){
      ...
      return {handled:true,text:"👥 *GROUP INFO*\n\nName: "+meta.subject+"\nMembers: "+(meta.participants||[]).length+"\nAdmins: "+(meta.participants||[]).filter(x=>x.admin).length};
    }
    if(name==="admins"){
      const meta=await sock.groupMetadata(jid);
      const admins=(meta.participants||[]).filter(x=>x.admin).map(x=>"• "+x.id).join("\n");
      return {handled:true,text:"👑 *GROUP ADMINS*\n\n"+(admins||"None")};
    }
    if(name==="link"){
      const code=await sock.groupInviteCode(jid);
      return {handled:true,text:"🔗 https://chat.whatsapp.com/"+code};
    }
    if(name==="revoke"){
      const code=await sock.groupRevokeInvite(jid);
      return {handled:true,text:"♻️ Invite link revoked.\n\nNew link: https://chat.whatsapp.com/"+code};
    }
    if(name==="setname"){
      const value=textArg(args);if(!value)return {handled:true,text:"Usage: .setname <name>"};
      await sock.groupUpdateSubject(jid,value);return {handled:true,text:"✅ Group name updated."};
    }
    if(name==="setdesc"){
      const value=textArg(args);if(!value)return {handled:true,text:"Usage: .setdesc <description>"};
      await sock.groupUpdateDescription(jid,value);return {handled:true,text:"✅ Group description updated."};
    }
    if(name==="tagall"||name==="hidetag"){
      const meta=await sock.groupMetadata(jid);
      const mentions=(meta.participants||[]).map(x=>x.id);
      const body=name==="hidetag"?(textArg(args)||"📢 Attention everyone"):("📢 "+mentions.map(x=>"@"+x.split("@")[0]).join(" "));
      return {handled:true,sendMessage:{text:body,mentions}};
    }
    const target=String(args[0]||"").replace(/^@/,"");
    const targetJ=targetJid(target);
    if(!targetJ)return {handled:true,text:"Usage: ."+name+" @member"};
    const action=name==="kick"?"remove":name;
    await sock.groupParticipantsUpdate(jid,[targetJ],action);
    return {handled:true,text:"✅ "+name+" completed for "+targetJ.split("@")[0]+"."};
  }
