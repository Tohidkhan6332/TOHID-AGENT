...
async function createRepo(name,description="",isPrivate=false){const r=await api().post("/user/repos",{name,description,private:Boolean(isPrivate),auto_init:true});return{name:r.data.full_name,private:r.data.private,default_branch:r.data.default_branch,url:r.data.html_url};}
...
module.exports={createRepo,listRepos,searchRepos,searchIssues,searchCommits,getFile,putFile,createBranch,issue,pull};