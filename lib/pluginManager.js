...
async function searchMarketplace(query=""){
  ...
  if(!q)throw new Error("Marketplace search query required.");
  const r=await axios.get("https://api.github.com/search/repositories",{params:{q:q+" tohid-agent-plugin",sort:"stars",order:"desc",per_page:8},timeout:15000,headers:{"Accept":"application/vnd.github+json","User-Agent":"TOHID-AGENT"}});
  return (r.data?.items||[]).map(x=>({name:x.full_name,description:x.description||"",stars:x.stargazers_count||0,updatedAt:x.updated_at,url:x.html_url,defaultBranch:x.default_branch||"main"}));
}
...
module.exports={ROOT,REGISTRY,isOwner,validateSource,testSource,installSource,updateSource,searchMarketplace,installFromUrl,installFile,load,enable,disable,remove,list,logs,getCapabilities,getCommand,dispatchCommand,dispatchMessage,boot};
