...
const KNOWN = new Set([
  ...
  "skills","health",
  "play","song","ytmp3","ytmp4","facebook","instagram","tiktok","twitter","pinterest","spotify","mediafire","apk",
  "search","image","weather","translate","short","qrgen","sticker","toimg","tomp3","tovideo",
  "ask","summarize","rewrite","vision","tts","stt","quiz","trivia","truth","dare","joke","quote","fact","8ball","ship","roast",
  "font","bold","italic","reverse","encode","decode","base64","kick","add","promote","demote","tagall","hidetag","groupinfo","admins","link","revoke","setname","setdesc",
  "restart","update","broadcast","listusers","listgroups","blockuser","unblockuser"
]);
...
function natural(text,prefix="."){
  ...
  // Common admin/developer command phrases.
  m=raw.match(/^(?:list|show) (?:all )?(?:plugins)$/i);
  if(m)return p+"plugin list";
  m=raw.match(/^(?:list|show) (?:workspace )?files(?: in)?(?:\\s+(.+))?$/i);
  if(m)return p+"file list"+(m[1]?" "+m[1]:"");
  m=raw.match(/^(?:read|show|open) (?:the )?file\\s+(.+)$/i);
  if(m)return p+"file read "+m[1];
  m=raw.match(/^(?:backup) (?:the )?file\\s+(.+)$/i);
  if(m)return p+"file backup "+m[1];
  m=raw.match(/^(?:show|list) (?:feature|features)$/i);
  if(m)return p+"feature list";
  m=raw.match(/^(?:enable|turn on) feature\\s+(.+)$/i);
  if(m)return p+"feature on "+m[1];
  m=raw.match(/^(?:disable|turn off) feature\\s+(.+)$/i);
  if(m)return p+"feature off "+m[1];
  m=raw.match(/^(?:list|show) (?:delegated )?owners$/i);
  if(m)return p+"owner list";
  m=raw.match(/^(?:show|list) (?:agent )?tasks$/i);
  if(m)return p+"tasks";
  m=raw.match(/^(?:show|list) missions$/i);
  if(m)return p+"missions";
  m=raw.match(/^(?:show|list) schedules$/i);
  if(m)return p+"schedules";
  m=raw.match(/^(?:run|start) workflow[:\\s]+(.+)$/i);
  if(m)return p+"workflow "+m[1];
