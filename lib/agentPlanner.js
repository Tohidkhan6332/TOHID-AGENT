...
const PROTECTED=new Set([
  "whatsapp_channel","whatsapp_group_status","github_write_file","github_create_branch","github_create_issue","github_create_pull_request",
  "heroku_restart_app","heroku_stop_app","heroku_start_app","heroku_scale_app","heroku_redeploy","heroku_rollback","heroku_cancel_build","heroku_set_config_var","heroku_delete_config_var","heroku_maintenance","heroku_delete_app","github_create_repository","deploy_project","hosting_manage"
]);

const READ_ONLY=new Set([
  "calculator","system_info","current_time","agent_plan",
  "whatsapp_baileys_capabilities","github_list_repositories","github_search_repositories","github_search_issues",
...
function label(name){
  if(String(name||"").startsWith("whatsapp_"))return String(name).replace(/^whatsapp_/,"WhatsApp ").replace(/_/g," ");
...
async function execute(name,args,runner){
  ...
  return {tool:name,result,verification,execution_ms:Date.now()-started,plan:meta};
}
