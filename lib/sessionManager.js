...
const MAX_COMPRESSED_BYTES=22*1024;
...
function encodeAuthDir(authDir){
  ...
  if(packed.length>MAX_COMPRESSED_BYTES)throw new Error("Session is too large for a portable SESSION_ID/config var. Generate a fresh session immediately after linking.");
...
function decodeSessionId(value){
  ...
  if(!encoded||encoded.length>32768)throw new Error("Invalid SESSION_ID size");
  let payload;
  try{payload=zlib.inflateRawSync(Buffer.from(encoded,"base64url"));}catch{throw new Error("Invalid or corrupted SESSION_ID");}
  if(payload.length>MAX_PAYLOAD_BYTES)throw new Error("SESSION_ID payload is too large");
  let parsed;
...
function restoreSessionId(value,authDir){
  ...
  for(const file of parsed.files){
    ...
    total+=data.length;
    if(total>MAX_PAYLOAD_BYTES)throw new Error("SESSION_ID payload is too large");
