...
const MAX_BUNDLE_BYTES=22*1024;
...
function encodeAuthDir(authDir){
  ...
  if(packed.length>MAX_BUNDLE_BYTES)throw new Error("Session is too large for a portable SESSION_ID/config var. Generate a fresh session immediately after linking.");
...
function decodeSessionId(value){
  ...
  if(payload.length>6*1024*1024)throw new Error("SESSION_ID payload is too large");
  let parsed;
