...
function start(manager,token){
  ...
  bot.onText(/^\/(?:pair|sessionpair)(?:\s+(.+))?$/i,async msg=>{
    const chatId=msg.chat.id;
    ...
    if(number.length<10||number.length>15)return bot.sendMessage(chatId,"Usage: /pair 919876543210 or /sessionpair 919876543210");
