...
function start(manager,token){
  ...
  bot.onText(/^\/pair(?:\s+(.+))?$/i,async msg=>{
    ...
    try{
      const s=manager.create({phone:number,mode:"pairing",ownerId:String(chatId),env:chatEnv.get(String(chatId))||{}});
  ...
  bot.onText(/^\\/qr(?:\\s+(.+))?$/i,async msg=>{
    ...
    try{
      const s=manager.create({mode:"qr",ownerId:String(chatId),env:chatEnv.get(String(chatId))||{}});
